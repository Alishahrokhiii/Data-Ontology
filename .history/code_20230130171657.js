...
function excelFileToJSON(file) {
  try {
    ...
    reader.onload = function (e) {
      ...
      // console.log(result)
      console.log(createElems(result))
      var elems = createElems(result)
      console.log(elems)
      maketree(elems);
...
var createElems = (nodeArray) => {
  var nodes = []
  var edges = []
  nodeArray.forEach((item, index) => {
    nodes.push({data: {id:item["data "]}})
  })
  

  nodeArray.forEach((item, index) => {
    edges.push({data : { source: item["data "], target: item.type }})
  })
